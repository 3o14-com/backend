...
import { type Thumbnail, makeVideoScreenshot, uploadThumbnail } from "../utils/media";
...
import { persistAccount, persistAccountByUri } from "./account";
...
> {
  ...
  const existingPost = await db.query.posts.findFirst({
    with: { account: { with: { user: true } }, mentions: true },
    where: eq(posts.uri, object.id.href),
  });
  if (options.skipUpdate && existingPost != null) return existingPost;
  if (existingPost != null && existingPost.account.user != null) {
    return existingPost;
  }
  ...
  const account =
    options?.account != null && options.account.uri === actor.id?.href
      ? options.account
  ...
  if (object.replyTargetId != null) {
    if (
      ...
      options.replyTarget.uri === object.replyTargetId?.href
    ) {
      ...
      const result = await db
        .select({ id: posts.id })
        .from(posts)
        .where(eq(posts.uri, object.replyTargetId.href))
        .limit(1);
  ...
  if (objectLink != null) {
    const result = await db
      .select({ id: posts.id })
      .from(posts)
      .where(eq(posts.uri, objectLink.href))
      .limit(1);
    if (result != null && result.length > 0) {
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: [posts.uri],
      set: values,
      setWhere: eq(posts.uri, object.id.href),
    });
  ...
  for await (const tag of object.getTags(options)) {
    if (tag instanceof vocab.Mention && tag.name != null && tag.href != null) {
      const account = await persistAccountByUri(
        db,
        tag.href.href,
        baseUrl,
        options,
...
export async function persistSharingPost(
  ...
  | (Post & {
    account: Account & { user: User | null };
    sharing:
    | (Post & { account: Account & { user: User | null } })
    | null;
  })
  | null
> {
  ...
  const existingPost = await db.query.posts.findFirst({
    with: {
      ...
      sharing: { with: { account: { with: { user: true } } } },
    },
    where: eq(posts.uri, announce.id.href),
  });
  if (existingPost != null) return existingPost;
  const actor = await announce.getActor(options);
  if (actor == null) return null;
  const account =
    options.account?.uri != null && options.account.uri === actor.id?.href
      ? options.account
      : await persistAccount(db, actor, baseUrl, {
        ...options,
        skipUpdate: true,
...
export async function persistPollVote(
  ...
  const post = await db.query.posts.findFirst({
    ...
    where: and(
      eq(posts.uri, object.replyTargetId.href),
  ...
  const voter = await persistAccountByUri(
    db,
...
export function toObject(
  ...
  return new cls({
    id: new URL(post.uri),
    attribution: new URL(post.account.uri),
    tos: [
      ...(post.visibility === "public"
        ? [PUBLIC_COLLECTION]
        : post.visibility === "private" && post.account.user != null
          ? [ctx.getFollowersUri(post.account.user.username)]
          : []),
      ...post.mentions.map((m) => new URL(m.account.uri)),
    ],
      ...
      ...(post.quoteTarget == null
        ...
        : [
          new Link({
            ...
            href: new URL(post.quoteTarget.uri),
            name:
              post.quoteTarget.url != null &&
                post.content?.includes(post.quoteTarget.url)
                ? post.quoteTarget.url
                : post.quoteTarget.uri,
          }),
        ]),
    ],
    replyTarget:
      post.replyTarget == null ? null : new URL(post.replyTarget.uri),
    replies: new OrderedCollection({
      id: new URL("#replies", post.uri),
      totalItems: post.replies.length,
      items: post.replies.map((r) => new URL(r.uri)),
    }),
    shares:
      post.sharesCount == null
        ? null
        : new Collection({
          id: new URL("#shares", post.uri),
    ...
    quoteUrl: post.quoteTarget == null ? null : new URL(post.quoteTarget.uri),
...
export function toAnnounce(
  ...
  return new vocab.Announce({
    id: new URL("#activity", post.uri),
    actor: new URL(post.account.uri),
    object: new URL(post.sharing.uri),
    published: toTemporalInstant(post.published),
    to:
      post.visibility === "public"
      ...
      post.visibility === "private"
        ...
        : [
          post.visibility === "public"
            ? ctx.getFollowersUri(handle)
            : vocab.PUBLIC_COLLECTION,
          new URL(post.sharing.account.uri),
        ],
  });
}
