...
import { featuredTags, posts } from "../../db/schema";
import { type Uuid, isUuid, uuidv7 } from "../../utils/uuid";
...
app.get("/", tokenRequired, scopeRequired(["read:accounts"]), async (c) => {
  ...
  const tags = await db.query.featuredTags.findMany({
    where: eq(featuredTags.accountId, user.account.id),
  });
  const stats = await getFeaturedTagStats(db, user.account.id);
  return c.json(
...
app.delete(
  ...
  async (c) => {
    ...
    const result = await db
      ...
      .where(
        and(
          eq(featuredTags.accountId, user.account.id),
...
async function getFeaturedTagStats(
  ...
  const result = await db
    ...
    .where(
      and(
        eq(featuredTags.accountId, accountId),
